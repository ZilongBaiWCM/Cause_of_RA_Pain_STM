...
unique_inflammation <- c('High: Lymphoid', 'Mixed: Myeloid','Low: Fibroid+Ungraded','All', 'Lymphoid+Fibroid+Ungraded')
#
pathotypes <- c('lymphoid', 'myeloid','fibroidungraded','all','lymfibun')
names(pathotypes) <- unique_inflammation
#
coloring <- c('purple', 'green', 'orange', 'blue', 'blue')
...
for(infla in unique_inflammation){
  ...
  gex_zscores <- read.csv(paste(data_dir, 'GEX_zscores_bulk_', patho, '_PEAC_v1.csv', sep=""), row.names = 1)
  ...
  #ggsave(plot = sp_ls.tsne, paste("/Users/baizilong/Documents/Dana-dataset/RA_paper/peac_validation_pain_genes_summary_scores_vs_VAS_",infla,"_Fig2_extended_v1.png",sep=""))
