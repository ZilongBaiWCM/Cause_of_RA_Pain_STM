...
if __name__ == '__main__':
    ...
    df_ps = pd.read_csv(data_dir + 'nuc_density_scores_all_patients.csv', index_col=0)
    print(df_ps)
    filter_condition = '_zscores_bulk_padj_<0.01log2FC_<0'
    df_gex = pd.read_csv(data_dir + 'GEX'+filter_condition+'_vs_nuc_all_patients.csv', index_col=0)
    print(df_ps.shape)
    print(df_gex.shape)
    print('Check zscores')
    ...
    df_ls.to_csv(save_dir + 'Laplacian_Scores' + filter_condition +'_vs_nuc_all_patients.csv')

    g = G.flatten()
    fig = plt.hist(g)
    plt.savefig(save_dir+"ave_nuc_similarity_histogram_all_patients.png")
    print(g.sort())
    print(g)
