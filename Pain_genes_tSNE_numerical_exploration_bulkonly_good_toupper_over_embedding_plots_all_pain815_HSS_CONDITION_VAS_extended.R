...
for(infla in unique_inflammation){
  ...
  gex_zscores <- read.csv(paste(data_dir, 'GEX_zscores_bulk_pain815_', inflat, '_HSS_CONDITION_VAS.csv', sep=""), row.names = 1)
  
  # Load pain scores
  vas_scores <- read.csv(paste(data_dir, 'pain_scores_hss_condition_vas_', inflat, '.csv', sep=""), row.names = 1)
  ...
  ggsave(plot = sp_ls.tsne, paste("/Users/baizilong/Documents/Dana-dataset/RA_paper_updated/hss_validation_pain_genes_summary_scores_vs_HSS_CONDITION_VAS_",infla,"_Fig2_extended.png",sep=""),
