...
#human_meta$celltype <- factor(human_meta$celltype, levels = unique(human_meta$celltype))
...
# Define a color palette using PuOr with the number of unique groups
custom_palette <- brewer.pal(num_groups, "RdBu")

# Set custom colors for each boxplot
custom_colors <- setNames(custom_palette, unique(human_meta$celltype))

# Create a boxplot with automatically set colors
CALCA_in_sorted <-  ggplot(human_meta, aes(x = celltype, y = calca_expression, fill = celltype)) + #, fill = celltype
  geom_boxplot(outlier.shape = NA) + #(outlier.shape = NA) +
  geom_jitter(color='black',width = 0.2, size=.3) + #,  aes(color = "b")
...
library(svglite)
ggsave(filename = paste(save_dir, 'FigS10_CALCA_hDRG.svg', sep=""), plot = CALCA_in_sorted, device="svg", width = 7.3, height = 5) #, width = 2.2, height = 2.9, dpi = 180, units = "in")
