...
df_dea <- read.csv(paste('/Users/baizilong/Documents/Dana-dataset/RA_paper_updated/', 'DEA_SC-F4_scf4_genes_sorted_genes.csv', sep=""), row.names = 1)
...
ggsave(filename = paste(save_dir, 'DoHeatmap_all_new_subtypes_scf4_genes_DeaRanked.pdf', sep=""), plot = heatmapfig, dpi = 300 ) #, width = 2.2, height = 2.9, dpi = 180, units = "in")

width <- 1600  # Increased width
height <- 1200  # Increased height

png(filename = paste(save_dir, 'DoHeatmap_all_new_subtypes_scf4_genes_DeaRanked.png', sep=""), width = width, height = height, res = 300)
print(heatmapfig)
dev.off()
