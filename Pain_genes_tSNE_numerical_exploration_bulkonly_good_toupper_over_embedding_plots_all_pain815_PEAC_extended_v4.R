...
vas_scores <- vas_scores[vas_scores$Inflammation.Level == 'High: lymphoid' | vas_scores$Inflammation.Level == 'Low: fibroid+ungraded',]
...
unique_inflammation <- append(unique_inflammation, 'all')

coloring <- c('purple', 'orange', 'blue')
...
for(infla in unique_inflammation){
  if (infla != 'all'){
    ...
    df_subsampled.tsne <- df_sampled.tsne
  }
  ...
  #ggsave(plot = sp_ls.tsne, paste("/Users/baizilong/Documents/Dana-dataset/RA_paper/peac_validation_pain_genes_summary_scores_vs_VAS_",infla,"_Fig2_v3.png",sep=""))
