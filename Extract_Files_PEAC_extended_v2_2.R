...
######
# TODO: Lymphoid + Myeloid
# Pre-processing to get PEAC patients with VAS pain scores in lymphoid/myeloid (combined high inflammation presented).
pain_scores_lymmye <- pain_scores[pain_scores$Pathotype=='lymphoid' | pain_scores$Pathotype=='myeloid', ]

write.csv(pain_scores_lymmye,'pain_scores_peac_lymmye.csv') # This file will be used to generate the pair-wise pain-based similarity graph.

gex_zscores_lymmye <- gex_zscores[, rownames(pain_scores_lymmye)]

gex_zscores_lymmye <- data.frame(gex_zscores_lymmye)

write.csv(gex_zscores_lymmye, 'GEX_zscores_bulk_lymmye_PEAC_v2.csv')
