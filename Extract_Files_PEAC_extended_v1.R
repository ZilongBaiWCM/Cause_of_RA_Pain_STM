...
rownames(pain_scores_lymphoid) <- pain_scores_lymphoid$Technology
...
gex_sub <- gex[row.names(gex) %in% gene_list, row.names(pain_scores_lymphoid)]

# Gene expression must be gene-wise z-scored over all the investigated patients within a given group.
gex_zscores_lymphoid <- apply(gex_sub, MARGIN = 1, FUN = scale )
gex_zscores_lymphoid <- t(gex_zscores_lymphoid)

colnames(gex_zscores_lymphoid) <- row.names(pain_scores_lymphoid)

gex_zscores_lymphoid <- data.frame(gex_zscores_lymphoid)

write.csv(gex_zscores_lymphoid, 'GEX_zscores_bulk_lymphoid_PEAC_v1.csv')
...
pain_scores_myeloid <- data.frame(pain_scores_myeloid[!duplicated(pain_scores_myeloid), ])

rownames(pain_scores_myeloid) <- pain_scores_myeloid$Technology

write.csv(pain_scores_myeloid,'pain_scores_peac_myeloid.csv') # This file will be used to generate the pair-wise pain-based similarity graph.
...
gex_sub <- gex[row.names(gex) %in% gene_list, row.names(pain_scores_myeloid)]

# Gene expression must be gene-wise z-scored over all the investigated patients within a given group.
gex_zscores_myeloid <- apply(gex_sub, MARGIN = 1, FUN = scale )
gex_zscores_myeloid <- t(gex_zscores_myeloid)

colnames(gex_zscores_myeloid) <- row.names(pain_scores_myeloid)

gex_zscores_myeloid <- data.frame(gex_zscores_myeloid)

write.csv(gex_zscores_myeloid, 'GEX_zscores_bulk_myeloid_PEAC_v1.csv')
...
pain_scores_fibroidungraded <- data.frame(pain_scores_fibroidungraded[!duplicated(pain_scores_fibroidungraded), ])

rownames(pain_scores_fibroidungraded) <- pain_scores_fibroidungraded$Technology
...
gex_sub <- gex[row.names(gex) %in% gene_list, row.names(pain_scores_fibroidungraded)]

# Gene expression must be gene-wise z-scored over all the investigated patients within a given group.
gex_zscores_fibroidungraded <- apply(gex_sub, MARGIN = 1, FUN = scale )
gex_zscores_fibroidungraded <- t(gex_zscores_fibroidungraded)

colnames(gex_zscores_fibroidungraded) <- row.names(pain_scores_fibroidungraded)

gex_zscores_fibroidungraded <- data.frame(gex_zscores_fibroidungraded)

write.csv(gex_zscores_fibroidungraded, 'GEX_zscores_bulk_fibroidungraded_PEAC_v1.csv')
...
pain_scores_all <- data.frame(pain_scores_all[!duplicated(pain_scores_all), ])

rownames(pain_scores_all) <- pain_scores_all$Technology
...
gex_sub <- gex[row.names(gex) %in% gene_list, row.names(pain_scores_all)]

# Gene expression must be gene-wise z-scored over all the investigated patients within a given group.
gex_zscores_all <- apply(gex_sub, MARGIN = 1, FUN = scale )
gex_zscores_all <- t(gex_zscores_all)

colnames(gex_zscores_all) <- row.names(pain_scores_all)

gex_zscores_all <- data.frame(gex_zscores_all)

write.csv(gex_zscores_all, 'GEX_zscores_bulk_all_PEAC_v1.csv')
...
pain_scores_myefib <- data.frame(pain_scores_myefib[!duplicated(pain_scores_myefib), ])

rownames(pain_scores_myefib) <- pain_scores_myefib$Technology
...
gex_sub <- gex[row.names(gex) %in% gene_list, row.names(pain_scores_myefib)]

# Gene expression must be gene-wise z-scored over all the investigated patients within a given group.
gex_zscores_myefib <- apply(gex_sub, MARGIN = 1, FUN = scale )
gex_zscores_myefib <- t(gex_zscores_myefib)

colnames(gex_zscores_myefib) <- row.names(pain_scores_myefib)

gex_zscores_myefib <- data.frame(gex_zscores_myefib)

write.csv(gex_zscores_myefib, 'GEX_zscores_bulk_myefib_PEAC_v1.csv')
...
pain_scores_fibroid <- data.frame(pain_scores_fibroid[!duplicated(pain_scores_fibroid), ])

rownames(pain_scores_fibroid) <- pain_scores_fibroid$Technology
...
gex_sub <- gex[row.names(gex) %in% gene_list, row.names(pain_scores_fibroid)]

# Gene expression must be gene-wise z-scored over all the investigated patients within a given group.
gex_zscores_fibroid <- apply(gex_sub, MARGIN = 1, FUN = scale )
gex_zscores_fibroid <- t(gex_zscores_fibroid)

colnames(gex_zscores_fibroid) <- row.names(pain_scores_fibroid)

gex_zscores_fibroid <- data.frame(gex_zscores_fibroid)

write.csv(gex_zscores_fibroid, 'GEX_zscores_bulk_fibroid_PEAC_v1.csv')
...
pain_scores_myefibun <- data.frame(pain_scores_myefibun[!duplicated(pain_scores_myefibun), ])

rownames(pain_scores_myefibun) <- pain_scores_myefibun$Technology
...
gex_sub <- gex[row.names(gex) %in% gene_list, row.names(pain_scores_myefibun)]

# Gene expression must be gene-wise z-scored over all the investigated patients within a given group.
gex_zscores_myefibun <- apply(gex_sub, MARGIN = 1, FUN = scale )
gex_zscores_myefibun <- t(gex_zscores_myefibun)

colnames(gex_zscores_myefibun) <- row.names(pain_scores_myefibun)

gex_zscores_myefibun <- data.frame(gex_zscores_myefibun)

write.csv(gex_zscores_myefibun, 'GEX_zscores_bulk_myefibun_PEAC_v1.csv')
...
pain_scores_lymfibun <- data.frame(pain_scores_lymfibun[!duplicated(pain_scores_lymfibun), ])

rownames(pain_scores_lymfibun) <- pain_scores_lymfibun$Technology
...
gex_sub <- gex[row.names(gex) %in% gene_list, row.names(pain_scores_lymfibun)]

# Gene expression must be gene-wise z-scored over all the investigated patients within a given group.
gex_zscores_lymfibun <- apply(gex_sub, MARGIN = 1, FUN = scale )
gex_zscores_lymfibun <- t(gex_zscores_lymfibun)

colnames(gex_zscores_lymfibun) <- row.names(pain_scores_lymfibun)

gex_zscores_lymfibun <- data.frame(gex_zscores_lymfibun)

write.csv(gex_zscores_lymfibun, 'GEX_zscores_bulk_lymfibun_PEAC_v1.csv')
