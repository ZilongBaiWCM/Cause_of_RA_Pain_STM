...
genes_lowinf <- df_genes$gene.name
rm(df_genes)
...
for (ctype in cell_types) {
  ...
  ggplot(data=de.markers, aes(x=avg_log2FC, y=-log(p_val_adj), col=DiffExp)) +
  ...
  fn <- paste(save_dir, 'DEA_',ctype,'_volcano_plot_figure3_nonpain_extended.png',sep="")
