...
#unique_inflammation <- c('High: Lymphoid', 'Mixed: Myeloid','Low: Fibroid+Ungraded','All', 'Lymphoid+Fibroid+Ungraded', 'Low:Myeloid+Fibroid', 'Low:Myeloid+Fibroid+Ungraded', 'High: Lymphoid+Myeloid')
unique_inflammation <- c('Low: Fibroid+Ungraded')
#
#pathotypes <- c('lymphoid', 'myeloid','fibroidungraded','all', 'lymfibun', 'myefib', 'myefibun', 'lymmye')
pathotypes <- c('fibroidungraded')
names(pathotypes) <- unique_inflammation
#
#coloring <- c('green', 'purple', 'blue', 'blue', 'blue', 'orange', 'orange', '#5A5A5A') # '#DC143C')
coloring <- c('blue')
...
# Pathotype: lymmye
# Graph mean degree: 15.45939206106982

#ud = 3
#degrees <- c(11,5,3,17,15,8,7, 15) 
degrees <- c(3) 
...
for(infla in unique_inflammation){
  ...
  genes <- setdiff(rownames(gex_zscores), rownames(pain_genes)) # Extract nonpain genes.
  ...
  sp_ls.tsne<-ggplot(df_subsampled.tsne.signed, aes(x, y)) +
    geom_point(aes(size=15), shape=15, show.legend=FALSE, color=coloring[infla], size=7) + 
  ...
  # if(infla == 'Low: Fibroid+Ungraded'){
  #   sp_ls.tsne <- sp_ls.tsne + scale_y_continuous(limits=c(-150, 150))
  # }
  
  print(range(df_subsampled.tsne.signed$x))
  
  ggsave(plot = sp_ls.tsne,
         filename=paste("/Users/baizilong/Documents/Dana-dataset/RA_paper_updated/peac_validation_nonpain_genes_summary_scores_vs_VAS_",infla,"_Fig2_extended_v2.png",sep=""),
