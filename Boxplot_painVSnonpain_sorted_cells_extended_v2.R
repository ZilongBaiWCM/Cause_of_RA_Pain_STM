...
zscores_pain <- apply(sc_pain_in_bulk_qc_tt, MARGIN = 1, FUN = scale )
zscores_pain <- t(zscores_pain)
rownames(zscores_pain) <- rownames(sc_pain_in_bulk_qc_tt)
colnames(zscores_pain) <- colnames(sc_pain_in_bulk_qc_tt)

zscores_nonpain <- apply(sc_nonpain_in_bulk_qc_tt, MARGIN = 1, FUN = scale )
zscores_nonpain <- t(zscores_nonpain)
rownames(zscores_nonpain) <- rownames(sc_nonpain_in_bulk_qc_tt)
colnames(zscores_nonpain) <- colnames(sc_nonpain_in_bulk_qc_tt)
...
for(ct in unique_cell_types){
  ...
  pain_gex <- zscores_pain[,tubes]
  nonpain_gex <- zscores_nonpain[,tubes]
...
fn <- "Bulk Sorted Cell QC passed RA Tissue pain vs nonpain boxplots v2.png"
...
ggsave(plot = p, 
       filename=paste(save_dir, "Pain vs Non-pain T test.png",sep=""),
       width = 12, height = 6, dpi = 180, units = "in", device='png')
...
for(ct in unique_cell_types){
  ...
  tubes <- rownames(df_celltypes[df_celltypes$Cell_Type==ct, ])
  mean_pain_gex_ct <- rowMeans(zscores_pain[, tubes])
  nontubes <- rownames(df_celltypes[!(df_celltypes$Cell_Type==ct), ])
  mean_pain_gex_nonct <- rowMeans(zscores_pain[, nontubes])
...
ggsave(plot = p, 
       filename=paste(save_dir, "One vs Others Pain Paired T test.png",sep=""),
       width = 12, height = 6, dpi = 180, units = "in", device='png')
...
for(ct in unique_cell_types){
  ...
  # Compute mean z-score of pain in ct
  tubes <- rownames(df_celltypes[df_celltypes$Cell_Type==ct, ])
  mean_nonpain_gex_ct <- rowMeans(zscores_nonpain[, tubes])
  nontubes <- rownames(df_celltypes[!(df_celltypes$Cell_Type==ct), ])
  mean_nonpain_gex_nonct <- rowMeans(zscores_nonpain[, nontubes])
...
ggsave(plot = p, 
       filename=paste(save_dir, "One vs Others Non-Pain Paired T test.png",sep=""),
       width = 12, height = 6, dpi = 180, units = "in", device='png')
...
ggsave(plot = p, 
       filename=paste(save_dir, "Pain vs Non-pain T test Alternative greater.png",sep=""),
       width = 12, height = 6, dpi = 180, units = "in", device='png')
...
ggsave(plot = p, 
       filename=paste(save_dir, "One vs Others Pain Paired T test Alternative greater.png",sep=""),
       width = 12, height = 6, dpi = 180, units = "in", device='png')
...
ggsave(plot = p, 
       filename=paste(save_dir, "One vs Others Non-Pain Paired T test Alternative greater.png",sep=""),
       width = 12, height = 6, dpi = 180, units = "in", device='png')
