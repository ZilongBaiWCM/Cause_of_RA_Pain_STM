...
if __name__ == '__main__':
    ...
    df_ps = pd.read_csv(data_dir + 'nuc_density_scores_all_patients_all_genes.csv', index_col=0)
    print(df_ps)
    df_gex = pd.read_csv(data_dir + 'GEX_zscores_bulk_vs_nuc_all_patients_all_genes.csv', index_col=0)
    print(df_ps.shape)
    print(df_gex.shape)
    print('Check zscores')
    ...
    df_ls.to_csv(save_dir + 'Laplacian_Scores_bulk_vs_nuc_all_patients_all_genes.csv')

    g = G.flatten()
    fig = plt.hist(g)
    plt.savefig(save_dir+"ave_nuc_similarity_histogram_all_patients_all_genes.png")
    print(g.sort())
    print(g)
